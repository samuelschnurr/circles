...
public partial class BoardTable
{
    ...
    private IEnumerable<Offer>? pagedData;
    private MudTable<Offer>? table;
    private int totalItems;
    private string? searchString;

    private async Task<TableData<Offer>> ServerReload(TableState state, bool useStaticData)
    {
        ...
        data = data!.Where(element =>
            {
                ...
                || (element.DeletedAt.HasValue && element.DeletedAt.Value.ToShortDateString().Contains(searchString, StringComparison.OrdinalIgnoreCase));
        ...
        switch (state.SortLabel)
        {
            ...
            case "createdAt_field":
                ...
                break;
            case "deletedAt_field":
                data = data.OrderByDirection(state.SortDirection, o => o.DeletedAt);
                break;
        }

        pagedData = data.Skip(state.Page * state.PageSize).Take(state.PageSize).ToArray();

        return new TableData<Offer>() { TotalItems = totalItems, Items = pagedData };
    }
